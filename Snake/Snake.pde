...
void checkBoundaries() {
  if(head.getY() == -10){
    head.setY(500);
  } else if(head.getY() == 510){
   head.setY(0); 
  } else if(head.getX() == -10){
    head.setX(500);
  } else if(head.getX() == 510){
   head.setX(0); 
  }
...
void drawTail() {
    ...
    for(int x = 0; x < segments.size(); x++){
 rect(segments.get(x).getX(), segments.get(x).getY(), 10,10);
    }
...
void checkTailCollision() {
  ...
  for(int x = 0; x < segments.size(); x++){
  if(head.getX() == segments.get(x).getX() && head.getY() == segments.get(x).getY()){
    fdEaten = 1;
